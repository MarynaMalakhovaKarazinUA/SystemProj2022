...
ring_buffer* buff_init(int size, int wrAmount, int reAmount) {
    ...
    buff->AmountElem = 0;
    buff->wrAmount = wrAmount;
    buff->reAmount = reAmount;
    buff->currentWriter = 1;
    buff->currentReader = 1;
...
void saveToFile(ring_buffer *temp) {
    int file;
...
void changeWriter(ring_buffer *temp) {
    int ch;
    
    printf("Оберіть письменника\n");
   
    for (int i = 1; i <= temp->wrAmount; i++) {
        printf("Письменник#%d\n", i);
    }
...
void changeReader(ring_buffer *temp) {
    int ch;

    printf("Оберіть читача\n");

    for (int i = 1; i <= temp->reAmount; i++) {
        printf("Читач#%d\n", i);
    }
    ...
    if(ch < 1 || ch > temp->reAmount) {
        printf("Помилка! Такого користувача не існує!\n");
        return;
    }

    temp->currentReader = ch;
}
...
int main() {
    ...
    printf("Введіть номер письменника:\n >>> ");

    if(scanf("%d", &wrAmount) < 0)
        printError();

    printf("Введіть номер читача:\n >>> ");

    if(scanf("%d", &reAmount) < 0)
        printError();

    ring_buffer *buff = buff_init(size, wrAmount, reAmount);
    ...
    while(1) {
        ...
        printf("Поточний читач:\nЧитач#%d\n", buff->currentReader);

        printf("\tГОЛОВНЕ МЕНЮ:\n1-Додати значення\n2-Показати значення\n3-Показати буфер\n4-Змінити письменника\n5-Змінити читача\n0-Вихід\n >>> ");
