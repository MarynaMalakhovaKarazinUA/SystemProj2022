...
ring_buffer* buff_init(int size, int writer_amount, int reader_amount) {
    ...
    buff->writer_amount = writer_amount;
    buff->reader_amount = reader_amount;
    buff->cur_writer = 1;
    buff->cur_reader = 1;
    return buff;
}
...
void changeWriter(ring_buffer *buf) {
    int choice;
    printf("Choose a writer\n");
    for (int i = 1; i <= buf->writer_amount; i++) {
        printf("writer#%d\n", i);
    }
    printf(">> ");
    if (scanf(" %d", &choice) < 0)
    ...
    if (choice < 1 || choice > buf->writer_amount) {
...
void changeReader(ring_buffer *buf) {
    int choice;
    printf("Choose a reader\n");
    for (int i = 1; i <= buf->reader_amount; i++) {
        printf("reader#%d\n", i);
    }
    printf(">> ");
    if (scanf(" %d", &choice) < 0)
    ...
    if (choice < 1 || choice > buf->reader_amount) {
        printf("There isn`t such user\n");
        return;
    }
    buf->cur_reader = choice;
}
...
int main() {
    ...
    if (scanf(" %d", &writer_amount) < 0)
        wrongInputError();
    printf("Enter the number of readers: ");
    if (scanf(" %d", &reader_amount) < 0)
        wrongInputError();
    ring_buffer *buff = buff_init(size, writer_amount, reader_amount);

    int choice = 0;

    while(1) {
        printf("Current writter: writter#%d\n", buff->cur_writer);
        printf("Current writter: reader#%d\n", buff->cur_reader);
        printf("1.Add value\n");
        printf("2.Get value\n");
        printf("3.Show the buffer\n");
        printf("4.Change writter\n");
        printf("5.Change reader\n");
